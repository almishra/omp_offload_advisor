...
#pragma omp target enter data map(to: weights[0:N], arrayX[0:N], arrayY[0:N]) \
                              map(to: xe, ye)
  gettimeofday(&tv2, NULL);
  start = (long)(tv1.tv_sec * 1000000 + tv1.tv_usec);
  end = (long)(tv2.tv_sec * 1000000 + tv2.tv_usec);
  fprintf(fp, "particle_kernel1,enter,%ld,%ld\n", sizeof(double)*N*3 + sizeof(double)*2,
          (end-start));
...
#pragma omp target exit data map(from: weights[0:N], arrayX[0:N], arrayY[0:N]) \
  ...
  fprintf(fp, "particle_kernel1,exit,%ld,%ld\n", sizeof(double)*N*3 + sizeof(double)*2,
          (end-start));
}
