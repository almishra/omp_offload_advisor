...
#if 0
void wilson_dslash_kernel(float (*chi_p)[LZ][LY][LX/2][4][3][2],
    float (*u_p_f_tst)[LT+1][LZ+1][LY+1][(LX/2)+1][4][3][3][2],
    float (*psi_p_f_tst)[LZ+2][LY+2][(LX/2)+2][4][3][2], int cb, FILE* fp)
{
#pragma omp target enter data \
  map(to:chi_p[0:LT][0:LZ][0:LY][0:LX/2][0:4][0:3][0:2]) \
...
#pragma omp target exit data \
  ...
  map(from:u_p_f_tst[0:2][0:LT+1][0:LZ+1][0:LY+1][0:LX/2+1][0:4][0:3][0:3][0:2]) \
  map(from:psi_p_f_tst[0:LT+2][0:LZ+2][0:LY+2][0:LX/2+2][0:4][0:3][0:2])
}
#endif
...
//    (double (*)[LZ][LY][LX/2][4][3][2]) malloc(sizeof(double)*LT*LZ*LY*LX/2*4*3*2);

  double (*frm_md_out_off)[LZ][LY][LX/2][4][3][2] =
    (double (*)[LZ][LY][LX/2][4][3][2]) malloc(sizeof(double)*LT*LZ*LY*LX/2*4*3*2);

  double (*g_md_field)[LT+1][LZ+1][LY+1][(LX/2)+1][4][3][3][2] =
    (double (*)[LT+1][LZ+1][LY+1][(LX/2)+1][4][3][3][2]) malloc(sizeof(double)*2*(LT+1)*(LZ+1)*(LY+1)*((LX/2)+1)*4*3*3*2);

  double (*frm_md_in)[LZ+2][LY+2][(LX/2)+2][4][3][2] =
    (double (*)[LZ+2][LY+2][(LX/2)+2][4][3][2]) malloc(sizeof(double)*(LT+2)*(LZ+2)*(LY+2)*((LX/2)+2)*4*3*2);
...
//  mem_from = sizeof(float)*(LT)*(LZ)*(LY)*(LX/2)*4*3*2;
  ...
  wilson_dslash_kernel(frm_md_out_off, g_md_field, frm_md_in, cb, fp);
