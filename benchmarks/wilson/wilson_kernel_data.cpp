...
#pragma omp target enter data \
  ...
  fprintf(fp, "wilson_kernel_data,enter,%ld,%ld\n", sizeof(double)*(2*(LT+1)*(LZ+1)*(LY+1)*(LX/2+1)*4*3*3*2 + (LT+2)*(LZ+2)*(LY+2)+(LX/2+2)*4*3*3*2), (end - start));
...
#pragma omp target exit data \
  ...
  fprintf(fp, "wilson_kernel_data,exit,%ld,%ld\n", sizeof(double)*(LT*LZ*LY*LX/2*4*3*2),
      (end - start));
}
